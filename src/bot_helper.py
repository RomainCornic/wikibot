...
def screenshot(page: Page, name: str):
    # TODO delete sinon j'aurais trop de screenshots mdr et c'est foutu apres
...
def safe_goto(page: Page, url: str, retries: int = 3, timeout: int = TIMEOUT):
...
def open_paquet(page: Page):
    ...
    btn_open.wait_for(state="visible", timeout=TIMEOUT)
    btn_open.click()

    for _ in range(4):
        delay = random.randint(50, 250)
        page.wait_for_timeout(delay)
        btn_next.wait_for(state="visible", timeout=TIMEOUT)
        btn_next.click()
...
def open_all_paquets(page: Page) -> dict:
    ...
    page.get_by_role("link", name="Paquets").wait_for(state="visible", timeout=TIMEOUT)
